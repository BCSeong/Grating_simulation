...
| `final_result` | `ndarray` or `None` | `(H, W)` | `inv_sqr_raw * warped_image`. `run()` 또는 GUI 핸들러에서 설정됨 |
...
### `compute_pixel_to_camera_distance_and_scale(progress_callback=None)`
...
| 이름 | 타입 | 기본값 | 설명 |
|---|---|---|---|
| `progress_callback` | `callable` or `None` | `None` | 진행률 콜백. 시그니처: `callback(current: int, total: int, message: str)`. 6단계(Setup, Flat d0, Flat d1, Tilted d0, Tilted d1, Scale & attenuation)에서 호출됨. 취소 시 `CancellationError` 발생 |
...
> **참고 (`attenuation_map` 미적용):** `attenuation_map`(cos⁴θ)은 계산되지만 `final_result`에 **적용되지 않습니다**. 이미징 측이 텔레센트릭 렌즈를 사용하므로 FoV 위치에 따른 밝기 변동이 없기 때문입니다. Non-telecentric 이미징 시나리오에서는 `attenuation_map`을 `final_result`에 곱하여 사용하십시오.
...
### `_compute_distance_and_scale_gpu(torch, progress_callback=None)`
...
| `progress_callback` | `callable` or `None` | `None` | 진행률 콜백. 래퍼에서 전달됨 |

---

### `_compute_distance_and_scale_cpu(progress_callback=None)`
...
보정된 이미지와 교차 단면(cross-section)을 matplotlib figure로 생성합니다. `final_result`(보정된 이미지)가 존재하면 이를 표시하고, 없으면 `warped_image`로 fallback합니다.
...
**Figure 구성:**
- `[0,0]` 메인: `final_result` 또는 `warped_image`의 크롭 이미지 (mm 단위 축)
...
8. `final_result = inv_sqr_raw * warped_image`
9. `imshow_result()`
...
1. **1. Load Image** → 프로젝션 BMP 로드
   - **Stretch to Square**: QCheckBox (기본값: 체크됨). 체크 시 이미지를 정사각형으로 리사이즈한 후 로드
...
6. **5. Compute Brightness** → 역제곱 + cos⁴ 밝기 감쇠 계산, `final_result` 생성. 진행률 다이얼로그 표시 (취소 가능)
